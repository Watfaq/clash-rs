...
//                 // I perfer to use a Result or ControlFlow enum here to
// indicate success or failure instead of panicking                 // Such as:
// ControlFlow::Continue(()) or ControlFlow::Break(error)                 // We
// can cancel ourself if it's an error, or just log it and return if it's a
// success                 // if output.is_break() {
//                     // token.cancel();
//                 // }
...
//             // For TUIC we need send `FIN` to the peer before close the
// connection,             // so we can do that here Or close all UdpSesions

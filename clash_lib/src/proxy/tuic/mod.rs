...
impl Handler {
    ...
    async fn get_conn(
        ...
        // if mark not match the one current used, then reconnect
        ...
        let fut = async {
            ...
            if guard.is_none() {
                // init
                *guard = Some(self.ep.connect(resolver).await?);
            }
            let conn = guard.take().unwrap();
            let conn = if conn.check_open().is_err() || reconnect {
                // reconnect
                self.ep.connect(resolver).await?
            } else {
                conn
            };

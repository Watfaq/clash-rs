...
impl TuicEndpoint {
    pub async fn connect(&self, resolver: &ThreadSafeDNSResolver) -> Result<Arc<TuicConnection>> {
        ...
        let connect_to = async {
            ...
            // if client and server don't match each other, then rebind local socket

use shadowquic::config::{default_alpn, default_congestion_control, default_initial_mtu, default_min_mtu, default_over_stream, default_zero_rtt};

use crate::{config::proxy::OutboundShadowQuic, proxy::shadowquic::{HandlerOptions,Handler}};
...
impl TryFrom<&OutboundShadowQuic> for Handler {
    ...
    fn try_from(s: &OutboundShadowQuic) -> Result<Self, Self::Error> {
        Ok(Handler::new(HandlerOptions {
            addr: format!("{}:{}",s.common_opts.server.to_owned(),s.common_opts.port),
            ...
            congestion_control: s.congestion_control.clone().unwrap_or(default_congestion_control()),

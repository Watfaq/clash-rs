...
for TARGET in $1; do
  ...
  echo "building for $TARGET(static link: $2)"
  ...
  output=./target/artifacts/clash-$TARGET
  if [ "$2" = "true" ]; then
    output=./target/artifacts/clash-$TARGET-static
  fi

  mv ./target/$TARGET/release/clash $output
done

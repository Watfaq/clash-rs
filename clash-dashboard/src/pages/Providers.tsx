...
import { getProxyProviders, getProxyDelay, updateProxyProvider, healthcheckProvider, getRuleProviders, updateRuleProvider, getRuleProviderRules } from '../lib/api';
...
export function Providers() {
  ...
  function RuleProviderRulesPanel({ name, behavior }: { name: string; behavior?: string }) {
    ...
    if (isLoading) {
      return <div className="text-[13px] py-2" style={{ color: '#8e8e93' }}>Loading rules…</div>;
    }

    if (isError) {
      return <div className="text-[13px] py-2" style={{ color: '#ff3b30' }}>Failed to load rules.</div>;
    }
    ...
    if (rules.length === 0) {
      return (
        <div className="text-[13px] py-2 italic" style={{ color: '#8e8e93' }}>
          {behaviorLower === 'classical'
            ? 'No rules loaded'
            : 'Rule entries are not enumerable for Domain/IPCIDR providers'}
        </div>
      );
    }
